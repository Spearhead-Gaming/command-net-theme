...
$denied = [];
$emptyTopics = true;
$emptyHtml = $twig->render('@Forumify/frontend/index.html.twig');
check(str_contains($emptyHtml, 'No discussions to display.'), 'empty topic lists have explicit states');
check(substr_count($emptyHtml, 'class="cn-empty cn-dashboard-empty"') === 1, 'one shared dashboard empty state is available to the unit filters');
if (isset($argv[3])) file_put_contents($argv[3] . '/empty.html', $emptyHtml);
$emptyTopics = false;
...
if (isset($argv[3])) file_put_contents($argv[3] . '/member.html', $html);
